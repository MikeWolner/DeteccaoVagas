...
#Delimitação das vagas no vídeo
vaga1 = [1, 89, 108, 2013]
...
#Definição de qual vídeo
video = cv2.VideoCapture('teste2.mp4')

#conversão de cores
while True:
    check, img = video.read()
    imgGrey = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    imgTh = cv2.adaptiveThreshold(imgGrey, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY_INV,25, 16)
    ...
    for x, y, w, h in vagas:
        ...
        #detectar quantidade de pixels brancos e mudar a cor
        if qtPxBranco > 3000:
